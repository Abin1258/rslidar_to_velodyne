...
#ifdef DEBUG
    pcl::PointCloud<velodyne_pcl::PointXYZIRT> velo_pc_per_ring[16];
#endif // DEBUG
    for(int i = 0; i < 16; i++)
    {
...
#ifdef DEBUG
    pcl::PointCloud<velodyne_pcl::PointXYZIRT>  velo_pc_filter;
    for(int i = 0; i < 16; i++)
    {
        pcl::PointCloud<velodyne_pcl::PointXYZIRT> velo_pc_per_ring_filter;
        for(int j = 1; j < velo_pc_per_ring[i].size()-1; j++)
        {
            velodyne_pcl::PointXYZIRT& last_point = velo_pc_per_ring[i][j-1];
            double range_before = sqrt(last_point.z*last_point.z + 
            last_point.x*last_point.x + last_point.y*last_point.y);

            velodyne_pcl::PointXYZIRT& cur_point = velo_pc_per_ring[i][j];
            double range_cur = sqrt(cur_point.z*cur_point.z + 
            cur_point.x*cur_point.x + cur_point.y*cur_point.y);

            velodyne_pcl::PointXYZIRT& next_point = velo_pc_per_ring[i][j+1];
